...
<div style="display:flex; gap:12px; align-items:center; margin-bottom:8px;">
  <label for="pm-layer-select">Layer:</label>
  <select id="pm-layer-select">
    <option value="old" selected>old</option>
    <option value="new">new</option>
  </select>
  <div style="flex:1"></div>
  <small>PMTiles from provided URL; options all reference the same dataset.</small>
</div>
<div id="ml-pmtiles-globe" style="height: 420px; margin-top: 0.5rem;"></div>
<div style="margin-top:8px; display:flex; gap:12px; align-items:center; flex-wrap: wrap;">
  <div id="ml-edr" style="font-size:12px; color:#333; white-space:pre-wrap;">Click on the map to fetch HM EDR time series…</div>
  <div style="flex:1"></div>
  <canvas id="ml-edr-chart" height="120" style="max-height:140px; width: 38vw; max-width: 480px;"></canvas>
  </div>
```
...
<script>
  ...
  function initMlPmGlobe() {
    ...
    map.on('style.load', () => {
      ...
      const layerOldId = 'pmtiles-old';
      const layerNewId = 'pmtiles-new';
      ...
      if (!map.getLayer(layerOldId)) {
        map.addLayer({ id: layerOldId, type: 'raster', source: srcOldId, paint: { 'raster-opacity': 0.95 } });
      }
      if (!map.getLayer(layerNewId)) {
        map.addLayer({ id: layerNewId, type: 'raster', source: srcNewId, paint: { 'raster-opacity': 0.95 } });
      }
      // Show 'old' by default, hide 'new'
      try {
        map.setLayoutProperty(layerOldId, 'visibility', 'visible');
        map.setLayoutProperty(layerNewId, 'visibility', 'none');
      } catch {}

      // Selector wiring: toggle between 'old' and 'new' layers
      const sel = document.getElementById('pm-layer-select');
      if (sel) {
        sel.addEventListener('change', () => {
          const val = sel.value;
          const showOld = (val === 'old');
          try {
            map.setLayoutProperty(layerOldId, 'visibility', showOld ? 'visible' : 'none');
            map.setLayoutProperty(layerNewId, 'visibility', showOld ? 'none' : 'visible');
