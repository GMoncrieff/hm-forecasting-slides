...
## Globe (MapLibre) {.no-scroll}

```{=html}
<link href="https://unpkg.com/maplibre-gl@3.6.2/dist/maplibre-gl.css" rel="stylesheet" />
<div id="ml-globe-wrap" style="position:relative;">
  <div id="ml-globe" style="height: 500px; margin-top: 1rem;"></div>
</div>
```

<script src="https://unpkg.com/maplibre-gl@3.6.2/dist/maplibre-gl.js"></script>
<script>
  let mlGlobeInited = false;
  function initMlGlobe() {
    if (mlGlobeInited) return;
    mlGlobeInited = true;
    ...
    map.on('load', () => {
      // Switch to globe projection once style is loaded
      if (map.setProjection) {
        map.setProjection({ name: 'globe' });
      }
      // Optional sky for nicer atmosphere
      if (!map.getLayer('sky')) {
        try {
          map.addLayer({
            id: 'sky',
          ...
          console.warn('Sky layer unsupported:', e);
        }
      }
      // Ensure proper sizing after transition
      setTimeout(() => map.resize(), 200);
      requestAnimationFrame(() => map.resize());
    });
  }

  function maybeInitMlGlobeOnCurrentSlide() {
    const current = window.Reveal && Reveal.getCurrentSlide ? Reveal.getCurrentSlide() : null;
    if (current && current.querySelector('#ml-globe')) {
      initMlGlobe();
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (window.Reveal) {
      Reveal.on('ready', maybeInitMlGlobeOnCurrentSlide);
      Reveal.on('slidechanged', maybeInitMlGlobeOnCurrentSlide);
    } else {
      initMlGlobe();
    }
  });
...
<div style="display:flex; gap:12px; align-items:center; margin-bottom:8px;">
  ...
  <div>BBox: <code id="rect-bbox">(draw to see bbox)</code></div>
  <div style="flex:1"></div>
  <small>Tip: Click "Draw Rectangle" then click-drag on the map.</small>
</div>
<div id="mb-basic-map" style="width:100%; height:540px; border:1px solid #ddd;"></div>

<script>
  // Minimal, inline init (no Reveal hooks)
  (function() {
    if (!window.mapboxgl) { console.error('mapboxgl not loaded'); return; }
    if (typeof mapboxgl.setTelemetry === 'function') {
      try { mapboxgl.setTelemetry(false); } catch {}
    }
    // Use global token or fallback to the one already set
    mapboxgl.accessToken = (window.MAPBOX_TOKEN || mapboxgl.accessToken || '');
    const map = new mapboxgl.Map({
      container: 'mb-basic-map',
      style: 'mapbox://styles/mapbox/light-v11',
      projection: 'mercator'
    });
    // Ensure proper sizing when the slide becomes visible
    const doResize = () => { try { map.resize(); } catch {} };
    window.addEventListener('resize', doResize);
    if (window.Reveal) {
      Reveal.on('ready', doResize);
      Reveal.on('slidechanged', () => {
        const el = document.getElementById('mb-basic-map');
        const vis = el && el.closest('section') && el.closest('section').classList.contains('present');
        if (vis) doResize();
      });
    }
    map.on('load', () => {
      // multiple passes to handle layout transitions
      doResize();
      requestAnimationFrame(doResize);
      setTimeout(doResize, 200);
      // Zoom to Africa after sizing
      map.fitBounds([
        [-20.0, -35.0], // SW
        [55.0, 38.0]    // NE
      ], { padding: 20, duration: 0 });

      // Lightweight rectangle draw (no external libs)
      const bboxEl = document.getElementById('rect-bbox');
      const btn = document.getElementById('rect-btn');
      let drawing = false;
      let startLngLat = null;
      const SRC_ID = 'rect-src';
      const FILL_ID = 'rect-fill';
      const LINE_ID = 'rect-line';
      ...
      function updatePolygonFromBounds(sw, ne) {
        ...
        if (bboxEl) {
          bboxEl.textContent = `${west.toFixed(6)}, ${south.toFixed(6)}, ${east.toFixed(6)}, ${north.toFixed(6)}`;
        }
      }
      ...
      function clearRect() {
        ...
        if (bboxEl) bboxEl.textContent = '(draw to see bbox)';
      }

      const onMove = (e) => {
        if (!drawing || !startLngLat) return;
        ensureLayers();
        updatePolygonFromBounds(startLngLat, e.lngLat);
      };
      const onUp = (e) => {
        ...
        drawing = false;
        if (startLngLat) updatePolygonFromBounds(startLngLat, e.lngLat);
      };
      ...
      if (btn) {
        btn.addEventListener('click', () => {
          ...
          // wait for first mousedown on map to set start
          const onDown = (e) => {
