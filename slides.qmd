...
Recognizing this trade-off, we do not attempt to forecasts HM at fine scale or far into the future. Instead, we focus on forecasting HM at 1km resolution for the period 2020-2040. 
</div>
...
<br><br>
Potential reuse includes (i) risk-based site selection for protected and conserved areas and other area-based measures aligned with GBF Target 1; (ii) prospective evaluation of conservation additionality by comparing observed threat outcomes to expected trajectories; (iii) integration as pressure inputs to biodiversity impact models (e.g., GLOBIO) and ecosystem-condition indicators; and (iv) screening in spatial planning and environmental assessment where forward-looking pressure maps can complement land-use scenarios.
</div>

## The model
<div style="font-size:0.5em">
### Overview
The overall structure of the model is to take both static and dynamic covariates and historical HM and convert these into tabular data, where each pixel is a row. This is then used to train a gradient-boosted tree model. The target for prediction is future HM at different time horizons. For example, if we are training a model using covariates and historical data on HM from 1990, 1995 and 2000, we can train one model to predict 5 years ahead (2005), another to predict 10 years ahead (2010), another 15 years ahead (2015), and another 20 years ahead (2020). After training these models, we can now give them the same covariates and any three successive HM observations (e.g. 2010, 2015, 2020) to produce predictions at these multiple time horizons (e.g. 2025, 2030, 2035, 2040). 

Important to note is that we do not use development scenarios or naartiaves to drive forecast. Rather, models are trianed in historical data and then use to preict the future. This can be thought of as using a business as usual scenario to drive the forecast. But business as-usual is not soley based on a individua sites hisotriy, but learnt from trying to predict the observed devlopment of other sites with simlar hisotires/covariates.
...
EarthENv provides cross-scale topographic data derived primarily from GMTED2010/SRTM and aggregated to standardized 1 km products. Variables used here: elevation, slope, aspect sine (“eastness”), aspect cosine (“northness”), and topographic position index (TPI; elevation relative to neighborhood mean).  

- **Location**  

Geographic coordinates are encoded using real spherical-harmonic basis functions following [Russwurm et al 2024](https://arxiv.org/abs/2310.06743). We encode each pixel’s latitude/longitude by projecting it onto a small set of smooth, globe-native “wave” patterns—spherical harmonics—so every place gets a stable, seam-free location fingerprint defined directly on the sphere. Compared with rectangular sin/cos encodings, spherical harmonics avoid artifacts at the dateline and poles because they are well-defined everywhere on the globe. Low orders capture only broad gradients (e.g., pole-to-equator), and increasing the order (L) adds finer spatial detail; in practice a modest order is picked, resulting in a fixed  compact feature vector (e.g., L =4 yields 25 features) that is computed once and fed to the model. <br>
![](/images/location.png)
...
- **Neighbourhood human modification (HM)**  
Each year, we calculate human modification (HM) at two neighborhood scales by averaging the HM values across hexagonal zones defined by the H3 grid system —a hierarchical, globe-covering grid that divides Earth into equal-area hexagons (with a few pentagons for closure) at multiple resolutions. Specifically, we use resolution 4 cells (≈ 1,770 km²  each) to capture broader regional context and resolution 6 cells (≈ 36 km² each) to capture local context. Every pixel is assigned the average HM of the hexagon it falls into at those resolutions, giving it a neighborhood-level context rather than just its local HM value. The model includes neighborhood HM values for both lagged and differenced HM overall covariates. <br>
 ![](/images/h3.png)
...
### Data splitting
The full dataset is stratified by the level 4 H3 index, and 10% of all primarily terrestrial H3 indices are set aside for model fitting, validation, calibration, and testing. Because the distribution of HM is skewed toward low values, we stratify this sample by the average HM at H3 level 4, and sample equally from cells with overall HM less than 0.1, between 0.1 and 0.4 and greater than 0.4. From this subset, 70% of the H3 indices are used for model fitting, 10% for validation, 10% for calibration, and 10% for testing. The validation set was used to assess model performance when tuning settings, the calibration set was used to calibrate uncertainty, and the testing set was used only once to estimate the accuracy of the final model. The image below shows how this partitioning would work over the globe <br>

![](/images/split.png)

### Fitting and Calibration 

Forecasts are produced in Python using MLForecast. MLForecast handles multi-step targets and lag/transform feature generation for each series. The base learner is LightGBM trained with the quantile objective.  
We fit separate models for each horizon (i.e. 5 years ahead, 10 years ahead, 15 years ahead, 20 years ahead) at α=0.50 (median) and α  ∈  {0.025, 0.975}, creating a separate prediction for the median, upper and lower quantiles. From these quantiles we obtain 95% prediction bands. The raw quantile bands are then calibrated (i.e. adjusted up or down) using a held-out calibration split to ensure that the coverage of prediction intervals matches the 95% target (the true value will fall within the prediction intervals in 95/100 samples on average) using conformal prediction.  We quantify point-forecast accuracy with mean absolute error. 
</div>

## Model performance
<div style="font-size:0.5em">
Comparing the fitted models against baseline assumptions - In the absence of a model we might assume either the status quo (zero change) or a linear trend. How does our model compare to these baselines over different horizons? In the figure below ‘Multihorizon’ represents the median prediction from the multihorizon model, ‘Zero’ assumes that future HM does not change for the last observed HM. ‘Linear’ assumes HM continues to increase at the same rate observed between the last  2 observations. 
 ![](/images/horizon.png)
 ...
 <br><br>
 How well can we predict HM in 20 years time? THe figure belpw shows modelled vs observed HM with a 20 year forecast horizon (i.e. predicting 2020 with data up to 2000). HM is scaled by 10000 (i.e. 1000 = 0.1 HM, 10000 = 1 HM). <br><br> ![](/images/abs_err.png)  <br><br>Very well!  <br><br>
 But remember, just assuming it wont change works pretty well, so predicting the absolute value is not that hard
 <br><br>
  How well can we predict how much HM will change in 20 years time?  Below we see modelled vs observed HM change over a 20 year forecast horizon (i.e. predicting change from 2000 to 2020 with data from 2000 to 1990). Again, HM is scaled by 10000. <br><br> ![](/images/diff_err.png)  <br><br> It is hard, but we get the overall direction and magnitude right
 <br><br>
</div>
...
<div style="font-size:0.5em">
What do individual pixels look like? Below we see how HM predicted from 2005 to 2020 based on data from 1990-2000, comparing predictions to observations. We see some in which it is quite certain they will remain intact and not change (first panel), Some that are high and expected to remain stable (second panel) some that are expected to reamin intact, but where loss of condition is a potential outcome (third panel), we suspect might become less intact. Latley we also see example where the a large change is observed, but the median prediction does not capture this. However, the wide uncertaitny band indicates that the models does not rule out this outcome (fourth panel). Including uncertainty captures the range of possible outcomes and helps us understand the risk of different scenarios.
 <br><br>
![](/images/timeseries.png)
  <br><br>
Large changes in HM or major industrail development are rare events. It is not unexpected that the model rarely predicted this to be the most likely outcome. What is encouraging is that the model helps us map where we cannot rule out that there will be a large change in the future. Likewise, areas with little uncertainty are those where we are very confident about the future. We might not always get the mean prediction correct - this is a very hard problem to predict. But we can get well calibrated ranges so that we are 95% certain the true outcome will lie within these bounds. 
</div>

## Maps of predicted change and uncertainty
...
<br>
HM is predicted to increasly broadly acorss the defreostiaotn frontier, hover we do not predict that deforestion or very large increases in HM are the most likeyl outcome. Even at the defreostation fonrtier, forest loss over a 20 year priod is raley more likley tha  not. However, the predicted HM increases indicated that the model agres that there region is at risk of deforetation.
<br>
...
When we look at the upper 95% confidence limit, we see that the model predicted that a very wide area is at risk of severse increase in HM. This highlights the challegne of predicting rare events that results in large changes in HM, like deforestation. THese changes seldom occur, even in areas at high risk. The model therefore predict a moderatin increase in HM that balances the risk of deforestation with the risk of not deforesting. In these case it is more informative to look at the upper 95% confidence limit to understand if drasdtic chagne is within the range of possible outcomes.
<br>
...
<br>
We can define a thrshold HM value above which we may beleive lands can no longer be assumed to be natural. Here we choose 0.1 HM as the threshold, and map the extent of intact lands in 2040 and intact lands lost between 2020 and 2040 based on HM modelled at differen confidence levels. Oragne indicated that loss of natural lands between 2020 and 2040 is the most likley outcome, Pink indicated that loss is possible under the worst case (upper 95%) scenario and purple indicates that land remain imact in 2040 even in the worst case scenario.
<br>

### Yucatán Peninsula

In the Yucatán Peninsula, forest cover is being lost mostly around expanding agriculture, pasture, and urban/tourism development. Drivers include changing land tenure, proximity to roads and markets, soil fertility, and economic incentives and government programmes for agricultural intensification.
![Yucatán Peninsula](/images/sm_ar.png)
<br>

#### Observed HM in 2020

<iframe frameborder="0" class="juxtapose" width="100%" height="506" src="https://cdn.knightlab.com/libs/juxtapose/latest/embed/index.html?uid=db3585c4-9487-11f0-ba1b-0e6f42328d7d"></iframe>
<br>

#### Observed HM in 2020 vs Predicted HM in 2040


<iframe frameborder="0" class="juxtapose" width="100%" height="507.15894039735093" src="https://cdn.knightlab.com/libs/juxtapose/latest/embed/index.html?uid=12690e44-9488-11f0-ba1b-0e6f42328d7d"></iframe>
<br>
Despite deforestatin and development occuring in many region of the Yucatan Peninsula in the hirstial period, in the median ofrecast the is little large scale change predicted. Some areas around major urban centers and the agricultural frontier show modest increases in HM.
...
The upper 95% confidence liimitnhighlights the potetnial for agricultural intensifycation and futher development around existing frontiers
...
![](/images/int_leg.png)
<br>
<iframe frameborder="0" class="juxtapose" width="100%" height="637.8305084745763" src="https://cdn.knightlab.com/libs/juxtapose/latest/embed/index.html?uid=111cd74e-9494-11f0-ba1b-0e6f42328d7d"></iframe>
<br>
AS in the example of Mato Grosso lrage regions are show as at risk of loss of natural lands in the worst case scenario. A relativly small area is at risk of loss of natural lands in the median scenario. A disitinct spatial pattern is also appraten, with regions of the south and west of the image experienin gretera loss of natural land in the worst case scenario. 
<br>

</div>

## Known artefacts 
These results should be considered preliminary and subject to change as the model is refined and validated. We are aware of a number of issues and artefacts described below:
...
![](/images/pole.png)
<br>
Most likely cause is encoding of lat/long using spherical harmonic. Potential fix is to increase harmonic order, change the way we encode lat/long or simply remove lat/long covariates. 

### 2. Hexagonal patterns introduced by H3 gridding 

Example from Amazon basin: 
<br>
![](/images/hex.png)
<br>
Caused by h3 covariates and reflects the underly hexagon strucutre of neighbourhoods. Potential fix is to use a more complex neighbourhood hood calculation, but this will greatly increase model runtime. 
...
Switch between years and lower, median or upper forecasts to expore data. Data from 1990-2020 is observed, and only avaialbe for th median (observed) values. From 2025 to 2040 values are forecastas and available at lower (2.5%), middle (median) and upper (97.5%) percentiles. You can click any point on the map to see the values for that location.
```{=html}
